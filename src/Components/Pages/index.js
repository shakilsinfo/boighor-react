...
import ContentLoader from '../Skeleton/ContentLoader';
...
function Index() {
	...
	const [fetchInitialContent, setInitialContent] = useState([]);
  ...
  const [totalContent, setTotalContent] = useState(0);
	...
	const [scrollBottom, setsetScrollBottom] = useState('none');
		...
		axios({
			    ...
			    const timing = setTimeout(() => {
			    	...
			    	const responseData = response.data.data;
						const indexOfLastContent = currentPage * 5;
					  const indexOfFirstContent = indexOfLastContent - 5;
					  const currentContent = responseData.slice(indexOfFirstContent, indexOfLastContent);
			      ...
			      setTotalContent(responseData.length);
    ...
    const indexOfLastContent = currentPage * contentPerPage;
  	...
  	const currentContent = homeData.slice(indexOfFirstContent, indexOfLastContent);
    return await currentContent;
  };

  const fetchData = async () => {
  	const pageNumber = Math.ceil(totalContent / contentPerPage);
  	const contentFromFetch = await fetchContentData();
  	
  	
  	setInitialContent([...fetchInitialContent, ...contentFromFetch]);
  	if (contentFromFetch.length === 0 || contentFromFetch.length < 2) {
      sethasMore(false);
    }
    setCurrentPage(currentPage + 1);
  ...
  return (
  	<div>
    	<InfiniteScroll
	      dataLength={fetchInitialContent.length} //This is important field to render the next data
	      next={fetchData}
	      hasMore={hasMore}
	    ...
	    >
	      {loading==true ? <HomeSkeleton /> :
      		...
      		fetchInitialContent.map((view,key) =>{
      			...
      			if(view.itemtype == 5){
	      			if(view.contents.length > 0){
	      				...
	      				return (
	      					<section className="hero-area hero-slider-2"  id={key}>
        ...
        <section className="mt--10" style={{display:display==1?'block':'none'}}>
				...
				</section>
      	{
      		...
      		fetchInitialContent.map((fetchData,key) =>{
      			key *= 4;
						...
						if((key == 4)){
							if( catcode == 'plr'){
								...
								if(fetchData.contents.length >0){
									return (
										<section className="bg-gray  mt--30 section-margin"  key={key}>
